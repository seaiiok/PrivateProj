...
func main(){
    baseinfo := CertInformation{Country: []string{"CN"}, Organization: []string{"MES-shucai"}, IsCA: true,
    OrganizationalUnit: []string{"MES-Work"}, EmailAddress: []string{"seaii@qq.com"},
    Locality: []string{"ChengDu"}, Province: []string{"SiChuang"}, CommonName: "MES-Work",
    CrtName: "server.crt", KeyName: "server.key"}
...
func newCertificate(info CertInformation) *x509.Certificate {
    return &x509.Certificate{
        ...
        NotAfter:              time.Now().AddDate(30, 0, 0),//证书的结束时间 
