...
func (p *Proto) sqlStringMakeServerDTimes(ostr string) {
	_ostr := strings.Split(ostr, "\\")
	if len(_ostr) != 2 {
		p.body.MyConf[Proto_SQL_serverdbcol] = ostr
	}
	p.body.MyConf[Proto_SQL_serverdbcol] = _ostr[0] + sqlStringMakeDTime(_ostr[1])
}

func (p *Proto) sqlStringMakeClientsDTimes(ostr string) {
	_ostr := strings.Split(ostr, "\\")
	if len(_ostr) != 2 {
